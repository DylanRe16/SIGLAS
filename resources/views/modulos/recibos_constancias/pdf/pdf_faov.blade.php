...
<body>
    ...
    <div class="content">
        ...
        prestó sus servicios en este Organismo bajo la figura de <span class="bold uppercase">{{ $figura }}</span> 
        hasta @if($fecha_egreso_raw) el @endif <span class="bold">{{ $fecha_egreso_fmt }}</span> y cotizó el <span class="bold">Fondo de Ahorro Obligatorio para la Vivienda (FAOV)</span> 
        desde el <span class="bold">{{ $fecha_ingreso }}</span> hasta @if($fecha_egreso_raw) el @endif <span class="bold">{{ $fecha_egreso_fmt }}</span>, 
        en el <span class="bold">Banco Nacional de la Vivienda y Hábitat (BANAVIH)</span>.
    </div>

    <div class="content">
        Referencia que se expide a petición de la parte interesada en la ciudad de Caracas a los <span class="bold">{{ $dia }}</span> 
